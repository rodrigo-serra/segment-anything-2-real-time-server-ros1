...
class HappyposeServer():
    ...
    def getDetectionResults(self):
        ...
        try:
            ...
            self.depth_sub = message_filters.Subscriber(self.cam_depth_input_topic, Image)  # Subscribe to depth image
    ...
    def detectronSynchronizedCallback(self, detectronMsg, imgMsg, depthMsg):
        """Callback for synchronized detection and camera/depth image messages."""
        ...
        try:
            ...
            # Convert ROS Image message to OpenCV format for RGB
    ...
    def get_pose_from_server(self):
        """Fetch pose estimate from external Happypose server over HTTP (with JSON and images)."""
        try:
            ...
            # Convert RGB image to byte array using OpenCV
            _, img_encoded = cv2.imencode('.jpg', self.rgb_img)
            img_data = io.BytesIO(img_encoded.tobytes())

            # Convert depth image to byte array using OpenCV
            _, depth_encoded = cv2.imencode('.png', self.depth_img)
            depth_data = io.BytesIO(depth_encoded.tobytes())

            # Prepare the files to send in the request
            files = {
                'rgb_img': ('image.jpg', img_data, 'image/jpeg'),  # Image file
                'depth_img': ('depth_image.png', depth_data, 'image/png'),  # Depth image file
